...
import {
  DataAnalytics,
  GroupData,
} from "../../containers/PlotBuilder/Model/data.model";
...
const Consolidation: React.FC<IConsolidation> = (props) => {
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < props.groupData.length; index++) {
      if (props.postData[index].length > 1) {
        const young_value = props.postData[index].find(
          (p) => p.name === "young"
        )
          ? props.postData[index].find((p) => p.name === "young").value
          : 0;
        const strain_value = props.xunit === "POURCENT" ? 1.0 : 0.01;
        parameterValuesInit.push({
          curve: props.groupData[index].title,
          parameter: "young",
          value: young_value,
        });
        parameterValuesInit.push({
  ...
  const onClickAdjustHandler = () => {
    ...
    if (
      (algo === "failure" && checkedFailureCurves.length === 0) ||
      (algo === "stiffness" && checkedStiffnessCurves.length === 0)
    )
      return;
  ...
  const onChangeParameterHandler = (
    event: any,
    curveName: string,
    paramName: string
  ) => {
    let checkedCurvesUp = checkedStiffnessCurves;
    checkedCurvesUp = checkedCurvesUp
      .filter((e) => e !== curveName)
      .concat([curveName]); // Add only if not yet in
    setCheckedStiffnessCurves(checkedCurvesUp);

    let parameterValuesUp = parameterValues;
    const elt = parameterValuesUp.find(
      (e) => e.curve === curveName && e.parameter === paramName
    );
    if (elt) {
      elt.value = event;
    } else {
      parameterValuesUp.push({
        curve: curveName,
        parameter: paramName,
        value: event,
      });
  ...
  const parameterValue = (curve_name: string, param_name: string): number => {
    const elt = parameterValues.find(
      (e) => e.parameter === param_name && e.curve === curve_name
    );
    if (elt) return elt.value;
    else return 0;
  };
        ...
        >
          <TabPane tab="Failure" key="1">
            <Divider orientation="left" style={{ fontSize: "12px" }}>
              Select 2 or 3 averaged curves
            </Divider>
            {props.groupData.map((g, index) => {
              if (props.listAvg[index]) {
                const dis =
                  props.selectedCurves.findIndex((e) => e === g.title) === -1
                    ? false
                    : true;
                const checked =
                  checkedFailureCurves.findIndex((e) => e === g.title) === -1
                    ? false
                    : true;
                return (
                  <Row key={"row" + index}>
          ...
          <TabPane tab="Stiffness" key="2">
            ...
            >
              ...
              <Col span={8}>Targeted Young</Col>
              <Col span={8}>Strain Range</Col>
            </Row>
            {props.groupData.map((g, index) => {
              if (props.postData[index].length > 1) {
                return (
                  <Row style={{ fontSize: "12px" }} key={g.title}>
                    ...
                    <Col span={8}>
                      <InputNumber
                        ...
                        step={
                          parameterValue(g.title, "young") !== 0
                            ? Math.pow(
                                10,
                                Math.floor(
                                  Math.log10(
                                    Math.abs(parameterValue(g.title, "young"))
                                  )
                                ) - 1
                              )
                            : 1
                        ...
                        onChange={(event: any) =>
                          onChangeParameterHandler(event, g.title, "young")
                        }
                      />
                    </Col>
                    <Col span={8}>
                      <InputNumber
                        ...
                        step={0.01}
                        onChange={(event: any) =>
                          onChangeParameterHandler(event, g.title, "strain")
                        }
                      />
                    </Col>
                  </Row>
