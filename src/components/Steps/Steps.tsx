...
const Steps: React.FC<StepsProps> = (props) => {
  ...
  const changeParametersHandler = () => {
    if (paramsRef.current.length === 0) return;
  ...
  const updatedCurveHandler = (action: string) => {
    ...
    } else return props.updatedCurve(action);
  };
  ...
  function DisplayStep(props) {
    ...
    props.operations.map((op, i) => {
      ...
      if (i !== 0) status_previous = props.operations[i - 1].status;
      ...
      steps.push(
        <Step
          ...
          changeSelectedMethod={(select: string) =>
            changeSelectedMethod2(select, op.action)
          }
      ...
      >
        ...
        >
          <Button
            ...
            type="primary"
            disabled={false}
            onClick={() => applyAllHandler() /*updatedCurveHandler('all')*/}
          >
            Apply All
          </Button>
        </div>
